...
class stealth_Archer:
    ...
    def deal_damage(self, hit):
        if(self.alive == False):
            return
        if(self.steathtime - time.time() > 0 ):
            self.health -= 0
        else:
            self.health -= hit
        if self.health <= 0:
...
class Healer:
    ...
    def healing(self, closest_troop, V):
        if(self.alive == False):
            return
        closest_troop.health += self.heal
        if(closest_troop.health > closest_troop.max_health):
            closest_troop.health = closest_troop.max_health

        troops = barbarians + archers + stealth_archers + balloons + dragons + KoQ
        for troop in troops:
            if(troop.alive == False):
                continue
            if(troop.position[0] == closest_troop.position[0] and troop.position[1] == closest_troop.position[1]):
...
def move_healers(V):
    troops = barbarians + archers + stealth_archers + dragons + balloons + KoQ
    for heal in healers:
        if(heal.alive == False):
            continue
        closest_troop,healer_position = search_for_closest_troop(heal.position)
        if(closest_troop == None):
            continue
        heal.move(healer_position, V)
        heal.healing(closest_troop,V)
...
def search_for_closest_troop(pos):
    ...
    for troop in troops:
        if(troop.alive == False):
            continue
        dist = math.sqrt(abs(troop.position[0] - pos[0])**2 + abs(troop.position[1] - pos[1])**2)
        if (dist <= 7):
    ...
    if(flag==1):
        if(closest_troop==None):
            return None, None
        healer_position = (pos[0],pos[1])
        return closest_troop, healer_position
    else:
        if(closest_troop==None):
            return None, None
        healer_position = (closest_troop.position[0] + 7*(closest_troop.position[0]-pos[0])/closest_dist),(closest_troop.position[1]+ (7*(closest_troop.position[1]-pos[1])/closest_dist))
        return closest_troop, healer_position
